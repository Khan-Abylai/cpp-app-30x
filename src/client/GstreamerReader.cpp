...
GstreamerReader::GstreamerReader(const CameraScope &cameraScope, bool useGPUDecode,
        ...
        : ILogger("Camera " + cameraScope.getCameraIp()),
          ...
          zmqServerIp{cameraScope.getZmqServerIp()} {
...
//    string streamingPipelineString = "filesrc location=../videos/test_video.mp4 ! rtph264depay ! h264parse ! nvh264dec ! gldownload !  videoconvert ! video/x-raw, format=(string)BGR ! appsink name=sink emit-signals=true ";
...
GstFlowReturn GstreamerReader::newDecodedSample(GstAppSink *appSink, gpointer data) {
    ...
    if (!currentFrame.empty() &&
        (!thisPointer->sensorCameraTrigger || thisPointer->sensorCameraTrigger->isValidFrame(currentFrame))) {
        thisPointer->frameQueue->push(make_unique<FrameData>(thisPointer->cameraIp, move(currentFrame.clone()),
...
void GstreamerReader::setSensorCameraTrigger(shared_ptr<SensorCameraTrigger> sensorTrigger) {
    sensorCameraTrigger = move(sensorTrigger);
}

const std::string &GstreamerReader::getZmqServer() const {
    return zmqServerIp;
}
