...
Detection::Detection(IExecutionContext *executionContext,
    ...
    LOG_INFO("Threshold: %f", detectionThreshold);
...
Detection::getLicensePlates(vector<float> lpPredictions, int frameWidth, int frameHeight) {

    vector<tuple<float, shared_ptr<LicensePlate>>> licensePlatesWithProbs;

    float scaleWidth = static_cast<float>(frameWidth) / IMG_WIDTH * PLATE_GRID_SIZE;
    float scaleHeight = static_cast<float>(frameHeight) / IMG_HEIGHT * PLATE_GRID_SIZE;
...
vector<shared_ptr<LicensePlate>> Detection::detect(const cv::Mat &frame) {
    ...
    vector<tuple<float, shared_ptr<LicensePlate>>> licensePlatesWithProbs = getLicensePlates(move(lpPredictions),
                                                                                             frame.cols, frame.rows);
    sort(licensePlatesWithProbs.begin(), licensePlatesWithProbs.end(), greater<>());
    return nms(licensePlatesWithProbs);
}

vector<shared_ptr<LicensePlate>> Detection::nms(const vector<tuple<float, shared_ptr<LicensePlate>>> &licensePlates) {
