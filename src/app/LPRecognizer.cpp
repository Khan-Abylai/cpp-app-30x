...
void LPRecognizer::createEngine() {
    ...
    auto builder = unique_ptr<IBuilder, TensorRTDeleter>(createInferBuilder(TensorRTEngine::getLogger()), TensorRTDeleter());
...
vector<float> LPRecognizer::executeEngine(const vector<cv::Mat> &frames) {
    ...
    predictions.resize(batchSize * OUTPUT_SIZE,0.0);
